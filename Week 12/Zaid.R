...
for (provider in esri) {
  l_aus <- DANmap %>% addProviderTiles(provider, group = provider)
}
...
l_dk <- leaflet() %>%   # assign the base location to an object
  setView(55.670249, 10.333328, zoom = 13)
...
DKkort <- leaflet() %>%
  setView(10.2209268,56.0887188, zoom = 6) %>%
  addMarkers(lng = places$Longitude,
             lat = places$Latitude,
             popup = paste(places$Description,
                           "<br>", places$Type))
DKkort
for(provider in esri) {
  DKkort <- DKkort %>% addProviderTiles(provider,group = provider)
}
DKkort1 <- DKkort %>%
  addLayersControl(baseGroups = names(esri),
                   options = layersControlOptions(collapsed = FALSE)) %>%
...
# Initialize DANmap with a Danish focus (e.g., centered on Copenhagen)
l_den <- leaflet() %>%
  setView(10.2209268,56.0887188, zoom = 6)
l_den
for(provider in esri) {
...
#Without clustering: Individual points visible (precise but cluttered at scale)
#With clustering: Dynamic grouped points (simplifies density, adapts on zoom).
# Your brief answer
...
leaflet() %>%
  ...
  addMarkers(lng = places$Longitude,
             lat = places$Latitude,
             popup = paste("<b>Description:</b>", places$Description,
                           "<br><b>Type:</b>", places$Type,
                           "<br><b>Notes:</b>", places$Notes,
                           "<br><b>Classification:</b>", places$Classification),
             clusterOptions = markerClusterOptions())  # Clustering added here!



######################################## CONGRATULATIONS - YOUR ARE DONE :)
library(htmlwidgets)
saveWidget(DKkort1, "map.html", selfcontained = TRUE)
